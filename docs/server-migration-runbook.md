...
После успешной проверки удалите файл дампа со всех мест, где он остался —
он содержит полные данные пользователей (включая хеши паролей):

```bash
rm paint_tracker-*.dump
```

Выполните на старом сервере, на новом сервере, и на локальной машине (если
дамп копировался через неё в шаге 8).
